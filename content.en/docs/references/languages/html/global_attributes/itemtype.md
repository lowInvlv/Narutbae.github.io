...
::: section-content
The [global attribute](../global_attributes) `itemtype` specifies the
...
[`location`](https://schema.org/location){target="_blank"} properties
specifying the concert\'s key details. In this case,
[`MusicEvent`](https://schema.org/MusicEvent){target="_blank"} would be
the URL used by `itemtype`, with `startDate` and location as
`itemprop`\'s which
[`MusicEvent`](https://schema.org/MusicEvent){target="_blank"} defines.
...
-   The item types must all be types defined in applicable
    ...
    [schema.org](https://schema.org/){target="_blank"}), and must all be
...
### Representing structured data for a product {#representing_structured_data_for_a_product}
...
<td>itemprop</td>
<td>description</td>
<td>Sleeker than ACME's Classic Anvil, the Executive Anvil is perfect
...
Tool](https://developers.google.com/search/docs/advanced/structured-data){target="_blank"}.
...
``` {signature="xs0NtuqHCCP+Qfk487VHIXmN3tnXyemKldYW3YZ2MMA=" data-language="html"}
...
or later.\
[https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/itemtype](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/itemtype){._attribution-link}
:::
