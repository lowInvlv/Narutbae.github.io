...
* [Basic Hooks](hooks-api-reference.md#basic-hooks)
  * [`useState`](hooks-api-reference.md#usestate)
  * [`useEffect`](hooks-api-reference.md#useeffect)
  * [`useContext`](hooks-api-reference.md#usecontext)
* [Additional Hooks](hooks-api-reference.md#additional-hooks)
  * [`useReducer`](hooks-api-reference.md#usereducer)
  * [`useCallback`](hooks-api-reference.md#usecallback)
  * [`useMemo`](hooks-api-reference.md#usememo)
  * [`useRef`](hooks-api-reference.md#useref)
  * [`useImperativeHandle`](hooks-api-reference.md#useimperativehandle)
  * [`useLayoutEffect`](hooks-api-reference.md#uselayouteffect)
  * [`useDebugValue`](hooks-api-reference.md#usedebugvalue)
  * [`useDeferredValue`](hooks-api-reference.md#usedeferredvalue)
  * [`useTransition`](hooks-api-reference.md#usetransition)
  * [`useId`](hooks-api-reference.md#useid)
* [Library Hooks](hooks-api-reference.md#library-hooks)
  * [`useSyncExternalStore`](hooks-api-reference.md#usesyncexternalstore)
  * [`useInsertionEffect`](hooks-api-reference.md#useinsertioneffect)
...
By default, effects run after every completed render, but you can choose to fire them [only when certain values have changed](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md).
...
However, not all effects can be deferred. For example, a DOM mutation that is visible to the user must fire synchronously before the next paint so that the user does not perceive a visual inconsistency. (The distinction is conceptually similar to passive versus active event listeners.) For these types of effects, React provides one additional Hook called [`useLayoutEffect`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md). It has the same signature as `useEffect`, and only differs in when it is fired.
...
> Note
>
> This only affects the timing of when the function passed to `useEffect` is called - updates scheduled inside these effects are still deferred. This is different than [`useLayoutEffect`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md), which fires the function and processes the updates inside of it immediately.
...
An alternative to [`useState`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md). Accepts a reducer of type `(state, action) => newState`, and returns the current state paired with a `dispatch` method. (If you’re familiar with Redux, you already know how this works.)

`useReducer` is usually preferable to `useState` when you have complex state logic that involves multiple sub-values or when the next state depends on the previous one. `useReducer` also lets you optimize performance for components that trigger deep updates because you can pass `dispatch` down instead of callbacks.

Here’s the counter example from the [`useState`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md) section, rewritten to use a reducer:
...
If you return the same value from a Reducer Hook as the current state, React will bail out without rendering the children or firing effects. (React uses the [`Object.is` comparison algorithm](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global\_Objects/Object/is#Description).)
...
Keep in mind that `useRef` _doesn’t_ notify you when its content changes. Mutating the `.current` property doesn’t cause a re-render. If you want to run some code when React attaches or detaches a ref to a DOM node, you may want to use a callback ref instead.
...
`useDeferredValue` only defers the value that you pass to it. If you want to prevent a child component from re-rendering during an urgent update, you must also memoize that component with `React.memo` or [`React.useMemo`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md):
...
The signature is identical to `useEffect`, but it fires synchronously _before_ all DOM mutations. Use this to inject styles into the DOM before reading layout in [`useLayoutEffect`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md). Since this hook is limited in scope, this hook does not have access to refs and cannot schedule updates.

> Note:
>
> `useInsertionEffect` should be limited to css-in-js library authors. Prefer [`useEffect`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md) or [`useLayoutEffect`](https://github.com/mindulle/Documents/blob/main/Languages/react/frequently-used/hooks/broken-reference/README.md) instead.
