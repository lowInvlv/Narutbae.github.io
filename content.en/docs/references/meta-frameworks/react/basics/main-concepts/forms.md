...
Specifying the `value` prop on a [controlled component](https://github.com/mindulle/Documents/blob/main/Languages/react/basics/main-concepts/broken-reference/README.md) prevents the user from changing the input unless you desire so. If you’ve specified a `value` but the input is still editable, you may have accidentally set `value` to `undefined` or `null`.
